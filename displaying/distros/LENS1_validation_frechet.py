...
from scipy.stats import gumbel_r as frechet
...
fit_frechet = frechet.fit(data)
mu_frechet = frechet.mean(*fit_frechet)
...
x_qq = np.sort(data)
y_frechet = frechet.ppf(q, *fit_frechet)

# ax 3: return period
y_rp = np.linspace(xmin, xmax, 1000)
x_rp = 1/frechet.sf(y_rp, *fit_frechet)
y_rp_frechet = frechet.isf(1/x_rp, *fit_frechet)
...
ax.plot(years, np.tile(mu_frechet, tl.size), c='#FC2947', ls='--', lw=0.8)
ax.plot(mu_clim.time.dt.year, mu_clim.values, c='#159895', lw=0.8)
ax.legend(['CR', 'frechet fit mean', '30yr mean'], ncol=4, loc=2)
...
ax.plot(x, frechet.pdf(x, *fit_frechet), c='#FC2947',
        linewidth=2, label='frechet fit')
...
ax.set_ylabel('Reference (frechet fit)')
ax.scatter(x_qq, y_frechet, ec='k', fc='#159895', s=20, alpha=0.5)
...
ax.plot(x_rp, y_rp_frechet, color='#FC2947', lw=1.5, label='frechet fit')
ax.legend()
...
filename = 'LENS1_validation_gumbel_r.png'
