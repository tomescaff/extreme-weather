...
x_qq = np.sort(data)
y_norm = norm.ppf(q, *fit_norm)

# ax 3: return period
y_rp = np.linspace(xmin, xmax, 1000)
x_rp = 1/norm.sf(y_rp, *fit_norm)
y_rp_norm = norm.isf(1/x_rp, *fit_norm)
...
ax.plot(years, np.tile(mu_norm, tl.size), c='#FC2947', ls='--', lw=0.8)
ax.plot(mu_clim.time.dt.year, mu_clim.values, c='#159895', lw=0.8)
ax.legend(['CR', 'Norm fit mean', '30yr mean'], ncol=4, loc=2)
...
ax.plot(x, norm.pdf(x, *fit_norm), c='#FC2947', linewidth=2, label='Norm fit')
...
ax.set_ylabel('Reference (Norm fit)')
ax.scatter(x_qq, y_norm, ec='k', fc='#159895', s=20, alpha=0.5)
...
ax.plot(x_rp, y_rp_norm, color='#FC2947', lw=1.5, label='Norm fit')
ax.legend()
...
filename = 'LENS1_validation_norm.png'
