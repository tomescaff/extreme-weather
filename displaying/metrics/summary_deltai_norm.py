...
lens1_hdi_be = np.zeros((n,))
lens1_hdi_lo = np.zeros((n,))
lens1_hdi_hi = np.zeros((n,))

lens1_fdi_be = np.zeros((n,))
lens1_fdi_lo = np.zeros((n,))
lens1_fdi_hi = np.zeros((n,))

for i, tau in enumerate(x):
    df = metrics.MLE_all_estimate_norm(df, model, Tg_ac=0.90, tau=tau)
    lens1_hdi_be[i] = df.loc['Best estimate', 'Delta ac-pa']
    lens1_hdi_lo[i] = df.loc['Lower estimate', 'Delta ac-pa']
    lens1_hdi_hi[i] = df.loc['Upper estimate', 'Delta ac-pa']

    lens1_fdi_be[i] = df.loc['Best estimate', 'Delta fu-ac']
    lens1_fdi_lo[i] = df.loc['Lower estimate', 'Delta fu-ac']
    lens1_fdi_hi[i] = df.loc['Upper estimate', 'Delta fu-ac']
...
ax = axs[0, 0]
ax.fill_between(x, lens1_hdi_lo, lens1_hdi_hi, color='#0B2447', alpha=0.2)
ax.plot(x, lens1_hdi_be, c='#0B2447', lw=2, label='Historical')
ax.fill_between(x, lens1_fdi_lo, lens1_fdi_hi, color='#FC2947', alpha=0.2)
ax.plot(x, lens1_fdi_be, c='#FC2947', lw=2, label='Future')
