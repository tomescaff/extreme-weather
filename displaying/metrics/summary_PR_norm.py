...
DE_lens1_hpr_be = np.zeros((n,))
DE_lens1_hpr_lo = np.zeros((n,))
DE_lens1_hpr_hi = np.zeros((n,))

DE_lens1_fpr_be = np.zeros((n,))
DE_lens1_fpr_lo = np.zeros((n,))
DE_lens1_fpr_hi = np.zeros((n,))

for i, tau in enumerate(x):
    df = metrics.DE_all_estimate_norm(lens_pa, lens_ac, lens_fu, model, tau)
    DE_lens1_hpr_be[i] = df.loc['Best estimate', 'PR ac-pa']
    DE_lens1_hpr_lo[i] = df.loc['Lower estimate', 'PR ac-pa']
    DE_lens1_hpr_hi[i] = df.loc['Upper estimate', 'PR ac-pa']

    DE_lens1_fpr_be[i] = df.loc['Best estimate', 'PR fu-ac']
    DE_lens1_fpr_lo[i] = df.loc['Lower estimate', 'PR fu-ac']
    DE_lens1_fpr_hi[i] = df.loc['Upper estimate', 'PR fu-ac']
...
ax = axs[0, 1]
ax.fill_between(x, DE_lens1_hpr_lo, DE_lens1_hpr_hi,
                color='#0B2447', alpha=0.2)
ax.plot(x, DE_lens1_hpr_be, c='#0B2447', lw=2, label='Historical')
ax.fill_between(x, DE_lens1_fpr_lo, DE_lens1_fpr_hi,
                color='#FC2947', alpha=0.2)
ax.plot(x, DE_lens1_fpr_be, c='#FC2947', lw=2, label='Future')
ax.grid(lw=0.2, ls='--', color='grey')
ax.set_axisbelow(True)
ax.legend(loc='center left', ncol=1)
ax.set_xlabel('Return period (yr)')
